...
int contains(Node*a, char*word) {
    if(a== NULL) return 0;
    if(strcmp(a->word,word) == 0) return 1;
    return contains(a->next, word);
}
...
int main()
{
    ...
    while(1) { 
        ...
        if(c == '\n' || c == EOF) {
            char* s = malloc(101);
            if(strcmp(w, s) != 0 && !contains(l1, w)) l1 = add(l1, w); 
            i = 0;
            break;
        }
    ...
    while(1) {
        c = getchar();
        // adding to list + breaking the loop
        if(c == EOF) {
            char* s = malloc(101);
            if(strcmp(w, s) != 0 && !contains(l2, w)) l2 = add(l2, w); 
            break;
        }
    ...
    //printf("\nl1:"); print(findFirst(l1));
    //printf("\nl2:"); print(findFirst(l2)); 
    Node* l3 = NULL;
    l1 = findFirst(l1); l2 = findFirst(l2);
    while(l1 != NULL) {
        if(contains(l2, l1->word)) {
            if(!contains(findFirst(l3), l1->word)) l3 = addA(l3, l1->word);
        }
        l1 = l1->next;
    }
